...
from .RuleBasedTeacher import rulebased_act, initialize_rule_based

from .RL_brain import PolicyNet
# Action space without the bomb action
ACTIONS = ['UP', 'RIGHT', 'DOWN', 'LEFT', 'WAIT', 'BOMB']

MODEL_NAME = 'network_parameters/save after 500.pt'
...
def act(self, game_state: dict) -> str:
    ...
    if self.train:
        if self.rulebased:
            return rulebased_act(self, game_state)
        else:
            # to choose action based on policy net
            features = state_to_features(self, game_state)
            probability = nn.functional.softmax(self.model.forward(features), dim=0)
            action = np.random.choice(ACTIONS, p=probability.detach().numpy())
            return action
    else:
        features = state_to_features(self, game_state)
        probability = nn.functional.softmax(self.model.forward(features), dim=0)
...
#     # combine the channels in (3,arena_channel.shape) and flatten it
#     hybrid_matrix = np.array([arena_channel, bomb_channel, agent_channel])
#     hybrid_array = hybrid_matrix.flatten()
#     return hybrid_array
